...
class BookSourceActivity : LifecycleActivity() {
    ...
    private fun showPopupMenu(position: Int, bookSource: BookSource) {
        ...
        if (bookSource.id > 0) items.add(0, R.string.score)
        ...
        BottomSelectorDialog(this, bookSource.name, items) { getString(it) }.callback { item ->
            when (item) {
                ...
                R.string.score -> showScoreDialog(position, bookSource)
     ...
     * 评分
     */
    private fun showScoreDialog(position: Int, bookSource: BookSource) {
        BookSourceScoreDialog(this, bookSource).callback { score ->
            controller.scoreBookSource(bookSource, score).observe(this, Observer {
                adapter.notifyItemChanged(position, bookSource)
            })
        }.show()
    }
